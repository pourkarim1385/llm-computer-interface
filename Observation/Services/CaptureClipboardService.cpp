...
ClipboardState CaptureClipboardService::getCurrentState() const {
    ...
    void CaptureClipboardService::captureImage_WIN(ClipboardState &state) const {
        ...
        // Spool to a temp .dib file NOW (clipboard closes after this call),
        // then keep only path + size resident; read back lazily.
...
void CaptureClipboardService::ensureGtk() {
    ...
    if (!inited) { gtk_init_check(nullptr, nullptr); inited = true; }
}

ClipboardState CaptureClipboardService::getCurrentState_LINUX() {
    ensureGtk();
    ClipboardState state;
    GtkClipboard* clip = gtk_clipboard_get(GDK_SELECTION_CLIPBOARD);
    if (!clip) return state;

    captureText(state, clip);
    captureImage(state, clip);
    return state;
}

void CaptureClipboardService::captureText_LINUX(ClipboardState& state, GtkClipboard* clip) {
...
void CaptureClipboardService::captureImage_LINUX(ClipboardState& state, GtkClipboard* clip) {
    if (!gtk_clipboard_wait_is_image_available(clip)) return;
    GdkPixbuf* pix = gtk_clipboard_wait_for_image(clip);
    if (!pix) return;

    // Spool to a temp png now, keep only the path resident, read lazily.
    ...
    if (size > 0)
        state.addItem(ClipboardItem::makeDeferred(
            ClipboardDataType::Image, "image/png", size, makeFileLoader(path)));
}
#endif
